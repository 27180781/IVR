...
בסיס נדרש: **Ubuntu 24.04 LTS** או Debian 12 ומעלה, 2 vCPU / 2GB RAM,
**IP ציבורי קבוע**. ה‑IP נכנס להגדרות טוויליו, אז הוא לא יכול להשתנות.

> **Ubuntu 22.04 לא יעבוד.** היא מביאה Asterisk 18, שבחבילה שלה אין ARI
> שמיש — וזה הממשק היחיד בין אסטריסק לאפליקציה. `bootstrap-server.sh`
> יסרב לרוץ על גרסה כזו ויגיד לך למה.
