...
לפני השיחה הראשונה, שווה לייצר צלילים זמניים:

```bash
sudo ./scripts/make-placeholder-sounds.sh
```

בלעדיהם השיחה תהיה שקטה, ולא תדעו אם האודיו באמת זורם או שהשיחה רק
התחברה — אלה שני דברים שונים לגמרי. הצלילים הם ביפים, לא הקלטות; תחליפו
אותם לפני עלייה לאוויר.
